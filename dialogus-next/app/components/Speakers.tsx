...
const SpeakerCard = ({
  ...
  >
    ...
    <div className="relative overflow-hidden rounded-2xl shadow-xl shadow-gray-200 transition-transform duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl aspect-[1/1.5]">
      <Image
        src={image}
        alt={name}
        width={400}
    ...
    <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-white/95 via-gray-50/90 to-white/95 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-center items-center p-4 sm:p-6 z-10 border border-gray-200">
      <div className="text-center max-w-xs sm:max-w-md">
        <h4 className="font-bold text-xl md:text-2xl text-fuchsia-600 mb-3 sm:mb-4 drop-shadow-sm">
          {name}
        </h4>
        <div className="space-y-2 sm:space-y-3">
          {intro.map((line, index) => (
            <p
              key={index}
              className="leading-relaxed text-gray-700 text-xs sm:text-sm"
            >
              {line}
            </p>
          ))}
    ...
    <div className="mt-4">
      <h3 className="font-bold text-base md:text-lg text-gray-900">{name}</h3>
    </div>
  </div>
);
